# SILARAH (?????) � Global Muslim Matrimony App
...
> SILARAH means "covenant" in Arabic. Every decision in this blueprint serves one purpose:
...
SILARAH solves all three. Premium design that respects the gravity of the decision. A structured, safe environment. Family built into the architecture, not bolted on.
...
Price must represent the same economic burden regardless of country. The formula: approximately 0.7 to 1.2 percent of average monthly income per region. This makes SILARAH accessible globally while maximizing revenue per market.
...
The SILARAH Arabic letterform (?????) appears with a covenant-inspired gold glow animation. The English wordmark "SILARAH" fades in below. The tagline "Begin with bismillah" appears. Two buttons: Create Profile (gold, primary) and Sign In (outlined, secondary). Total splash duration approximately 2.5 seconds before buttons appear.
...
Timeline:
  ...
  0ms    � Arabic letterform ????? at 0% opacity, scale 0.8
  300ms  � ????? fades to 100% opacity, scales to 1.0 (ease-out-cubic, 600ms)
  ...
  1000ms � "SILARAH" wordmark fades in below (0?100% opacity, 400ms)
...
The header shows the SILARAH wordmark on the left and a notification bell with a badge on the right. Below the header, a horizontal scrollable row of filter chips: All, Near Me, Sect, Deen, Age.
...
Non-subscriber men who try to open a chat see: "Subscribe to unlock messaging. Women always message free on SILARAH." The price shown is in their local currency.
...
Clean single-purpose screen. Header: "Unlock SILARAH." Subtext: "Women message free. Men subscribe to connect."
...
class SubscriptionService {
  static const _monthlyId = 'silarah_monthly';
  static const _annualId = 'silarah_annual';
...
To replace manual moderation, SILARAH uses Community Moderation.
...
BEGIN
  ...
  IF g = 'male' THEN
    ...
    IF sub = 'grace' AND expires IS NOT NULL AND expires > NOW() - INTERVAL '24 hours' THEN
      RETURN NEW;
    END IF;
    RAISE EXCEPTION 'Subscribe to unlock messaging. Women always message free on SILARAH.';
  END IF;
  RETURN NEW;
END; $$ LANGUAGE plpgsql;
...
- Join and observe 15�20 matrimony groups; contribute value; softly introduce SILARAH.
