...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) return error(401, "Authentication required.");
    ...
    if (userId !== user.id) return error(403, "You may only verify your own profile.");
    if (!dob || ageOn(dob) < 18) return error(422, "You must be at least 18 years old.");
    if (!Number.isFinite(similarity) || similarity < 0 || similarity > 1) {
      return error(400, "Invalid face similarity score.");
    }
    if (!idType || !/^[A-Z]{2}$/.test(countryCode)) return error(400, "Invalid document details.");
    if (!ownsKycPath(selfiePath, user.id, "kyc_selfie") || !ownsKycPath(idPath, user.id, "kyc_id")) {
    ...
    if (similarity >= likelyMatchThreshold) {
      ...
      return json({ status: "verified", message: "Your identity has been verified." });
    }
    ...
    if (similarity >= manualReviewThreshold) {
      ...
      if (updateError) throw updateError;
      return json({ status: "pending_review", message: "Your verification is pending review." });
    }
...
function ownsKycPath(path: string, userId: string, prefix: string): boolean {
  return new RegExp(`^${escapeRegExp(userId)}/${prefix}_[0-9a-f-]+\\.(jpg|jpeg|png|webp)$`, "i").test(path);
}
