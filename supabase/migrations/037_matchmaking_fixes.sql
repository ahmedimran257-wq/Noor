-- ============================================================
-- MIGRATION 037: MATCHMAKING LOCATION FIXES
-- Properly supports location_preference (sameCity, sameCountry, diaspora, openToAbroad)
-- and returns the full profile data structure required by the client.
-- ============================================================
...
BEGIN
  ...
  WITH combined_pool AS (
    ...
    SELECT
      ...
      p.open_to_diaspora,
      p.education_rank,
      -- Array columns from profiles directly (not in standard MV)
      p.languages,
      p.interests,
    ...
    SELECT
      ...
      (
        SELECT ph.blurhash FROM photos ph
    ...
    JOIN profiles p ON na.profile_id = p.id
  )
  SELECT
    cp.profile_id,
    cp.user_id,
  ...
  WHERE
    ...
    -- Location / diaspora preference / distance filter
    AND (
      -- 1. Explicit same city filter from UI
      (
        (p_filters->>'same_city')::boolean = true
        AND cp.city_id = v_profile.city_id
      )
      -- 2. Explicit same country filter from UI
      OR (
        (p_filters->>'same_country')::boolean = true
        AND cp.country_code = v_profile.country_code
      )
      -- 3. Explicit custom distance filter chosen in UI (overrides default onboarding location preference)
      OR (
      ...
      -- 4. Explicit "Anywhere" filter chosen in UI (bypasses default onboarding location preference)
      OR (
        (p_filters->>'anywhere')::boolean = true
      )
      -- 5. Default onboarding preferences (no custom UI overrides)
      OR (
        ...
        AND (
          ...
          -- B4. Open to Abroad (Default fallback: matches globally)
