-- Align signup consent and the server-authoritative reminder with policy 2.3.0.
-- Existing consent evidence remains immutable; existing members are prompted
-- to acknowledge the material billing and processor-disclosure clarification.
...
DECLARE
  ...
  v_digest text := encode(
    extensions.digest(
      convert_to(
        'silarah-launch-policy:2.3.0:terms|privacy|community|age|religious',
        'UTF8'
      ),
      'sha256'
    ),
...
BEGIN
  ...
  INSERT INTO private.policy_reminder_acknowledgements(
    ...
    v_user_id, '2.3.0', v_tx.accepted_at, now()
  )
...
AS $$
  SELECT
    ...
    (
      ...
      OR reminder.policy_version <> '2.3.0'
...
BEGIN
  IF trim(coalesce(p_policy_version, '')) <> '2.3.0' THEN
...
-- Keep export metadata current without duplicating the large, audited export
-- builder. Only the versioned public wrapper remains callable by members.
ALTER FUNCTION public.download_my_data(text) SET SCHEMA private;
ALTER FUNCTION private.download_my_data(text)
  RENAME TO build_personal_data_export;
REVOKE ALL ON FUNCTION private.build_personal_data_export(text)
...
DECLARE
  v_export jsonb;
BEGIN
  v_export := private.build_personal_data_export(p_client_version);
  RETURN jsonb_set(
    v_export,
    '{policy_version}',
    to_jsonb('2.3.0'::text),
...
END;
$$;
REVOKE ALL ON FUNCTION public.download_my_data(text) FROM PUBLIC, anon;
...
COMMENT ON FUNCTION public.download_my_data(text) IS
  'Authenticated rate-limited personal data export with current policy metadata.';
