-- ============================================================
-- MIGRATION 042: RESTORE FULL DISCOVERY FEED PAYLOAD
--
-- Migration 038 reintroduced user_id but narrowed the return table,
-- which broke client-side profile details, filters, and interest
-- sending. This restores the full shape from the matchmaking fixes
-- while keeping user_id in every returned candidate.
-- ============================================================
...
BEGIN
  ...
  IF COALESCE(v_sub_status, 'free') != 'active' AND p_page_size > 15 THEN
  ...
  WITH combined_pool AS (
    ...
    JOIN profiles p ON p.id = na.profile_id
  )
  SELECT
    cp.profile_id,
    cp.user_id,
