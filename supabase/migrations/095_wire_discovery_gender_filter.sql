-- Wire the discovery gender filter at the RPC boundary. The client already
-- exposes this filter, so the server must apply it instead of returning an
-- apparently unfiltered feed.
...
BEGIN
  ...
  SELECT
    ...
    r.blurhash
  FROM ranked AS r
  WHERE (v_gender_pref IS NULL OR r.gender = v_gender_pref)
    AND (
