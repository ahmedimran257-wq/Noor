-- Repair the profile-photo upload path contract after storage hardening.
--
-- Migration 148 used `\\.` inside a standard PostgreSQL string. The regex
-- engine interpreted that as a literal backslash followed by a wildcard,
-- so valid paths such as <user-id>/<object-id>.webp were rejected before a
-- signed upload URL could be issued. `[.]` expresses a literal dot without
-- depending on SQL string escaping.
