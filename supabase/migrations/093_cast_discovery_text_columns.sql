-- Cast discovery RPC text payload columns exactly to text. Prevents varchar/text return mismatches.
-- Fix open_to_diaspora source: it is projected by discovery_pool/new_arrivals, not profiles.
...
BEGIN
  ...
  SELECT
    ...
    p.gender::text,
    p.first_name::text,
    LEFT(p.last_name, 1)::text AS last_name_initial,
    ...
    p.bio::text,
    (
      SELECT CASE WHEN p.photo_privacy = 'public' THEN ph.storage_path::text ELSE NULL::text END
    ...
    p.last_active_at,
    p.location,
    p.marriage_timeline::text,
    pr.diaspora_mode,
    pr.open_to_diaspora,
    pr.preferred_countries::text[],
    pr.preferred_age_min,
    pr.preferred_age_max,
    p.education_rank,
    p.date_of_birth,
    p.previously_married::text,
    p.children_count,
    p.family_type::text,
    p.mother_tongue::text,
    p.community::text,
    p.living_expectation::text,
    p.quran_memorization::text,
    p.willing_to_relocate::text,
    p.is_boosted,
    p.boost_expires_at
  FROM public.profiles AS p
  LEFT JOIN public.cities AS c ON p.city_id = c.id
  LEFT JOIN public.profile_preferences AS pr ON p.id = pr.profile_id
  WHERE p.visibility = 'visible'
    ...
    AND NOT EXISTS (
      SELECT 1 FROM public.discovery_pool AS dp WHERE dp.profile_id = p.id
    );
END;
$$;
...
BEGIN
  ...
  SELECT
    ...
    END AS distance_km,
    cp.rank_score,
    cp.marriage_timeline::text,
    cp.height_cm,
    cp.complexion::text,
    cp.mother_tongue::text,
    cp.smoking_habit::text,
    cp.community::text,
    cp.diet_type::text,
    cp.living_expectation::text,
    cp.quran_memorization::text,
    cp.religious_education::text,
    cp.willing_to_relocate::text,
    cp.previously_married::text,
    cp.family_type::text,
    cp.children_count,
    cp.languages,
    cp.interests,
