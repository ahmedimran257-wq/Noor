  ...
  static const verification = LegalDocument(
    ...
    sections: [
      ...
      LegalSection('8. Regional availability and phone confirmation',
          'The initial service launch is limited to enabled launch markets shown in the app; phone confirmation is currently limited to enabled launch-country numbers. Silarah will never ask for your email or phone OTP outside the official in-app verification flow. Women do not need phone confirmation for free messaging. When a member deliberately continues with a paid Premium plan, SMS confirmation is completed before the store purchase. Referral-only promotional Premium does not require SMS confirmation. A phone badge appears only after successful SMS confirmation, never from a purchase alone. During an active paid Premium period, a confirmed number can be changed from Profile → Trust checks → Phone number using a new OTP. Changing it does not cancel or shorten the store subscription because billing remains tied to the account, not the phone number.'),
    ],
  );
