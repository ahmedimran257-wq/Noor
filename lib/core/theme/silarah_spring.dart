...
abstract final class SilarahSpring {
...
class SpringCurve extends Curve {
  const SpringCurve({
    this.spring = SilarahSpring.standard,
  ...
  double transformInternal(double t) {
    ...
    // Decoration and TextStyle lerps require values such as shadow blur radii
    // to stay non-negative. An under-damped spring can overshoot above 1.0;
    // reversing that lerp then produces an invalid negative radius.
    return simulation.x(timeInSeconds).clamp(0.0, 1.0).toDouble();
  }
}
