...
Migration parity: local, staging and production through migration `250`
...
The post-250 database counts reconcile to `2 + 1 + 6 + 206 = 215` warning-equivalent findings.
...
- [x] Local, staging and production migration history matches through 250.
- [x] Fresh version 2 post-250 production backup created at `supabase/backups/jukpscfxzwttgtxvrbmj/20260828T085814Z`; checksums, archive catalogue and 84-table snapshot inventory verified.
- [x] The post-250 production archive restored into the uniquely named staging-side database `silarah_restore_drill_20260828_090012`; all 84 archived table counts matched, and the exact ephemeral database was removed.
...
- [ ] Capture the refreshed post-250 Security and Performance Advisor dashboard totals for the release record.
